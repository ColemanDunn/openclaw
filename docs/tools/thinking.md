...
- For `xai/*`, fast mode requests `service_tier=priority` on public xAI and Grok OAuth text endpoints, preserving the model and reasoning level. Legacy model aliases keep their existing fast-model rewrites. See [xAI fast mode](/providers/xai#fast-mode).
